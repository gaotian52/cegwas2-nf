...
File Gposition = new File("${params.pos}")
Gposition_handle = Gposition.getAbsolutePath()
...
process eQTL_maps {
	...
	publishDir "${params.out}/eQTLmap", mode: 'copy', pattern: "eQTL_*"

	input:
    
    file(eQTL) from qtl_peaks
	val(fdr) from sig_threshold_map
	...
	Rscript --vanilla `which Run_eqtlmap.R` ${Gposition_handle} ${eQTL} ${fdr} ${t2g_handle}
