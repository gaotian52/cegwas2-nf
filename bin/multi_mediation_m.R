...
# load eqtl data
mqtl_infor <- read.delim(args[7], stringsAsFactors=FALSE)

meta_list <- mqtl_infor %>% 
  dplyr::select(gwtrait,m_chr,gwpeak,trait) %>% 
  distinct()
...
t_lgmtpm_gwas <- texpression_pheno %>% 
  dplyr::select(c(strain,unique(meta_list$trait))) %>% 
  dplyr::arrange(strain)  %>% 
  dplyr::select(-strain)
...
df_multi_med <- df_multi_transcript2 %>% 
  left_join(mqtl_infor,by=c("metabolite"="trait"))
