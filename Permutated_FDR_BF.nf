...
process rrblup_maps_perm {
	...
	input:
	set file("independent_snvs.csv"), file(geno), val(TRAIT), file(pheno), val(P3D), val(sig_thresh_bf), val(qtl_grouping_size), val(qtl_ci_size) from mapping_data_perm
